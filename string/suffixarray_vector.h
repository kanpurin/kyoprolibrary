#ifndef _SUFFIX_ARRAY_VECTOR_H_
#define _SUFFIX_ARRAY_VECTOR_H_
...
private:
    ...
    // 比較関数 s[si:] < t[ti:]
    bool lt_substr(std::vector< T >& t, int si = 0, int ti = 0)
    {
    ...
    // prefixがtの辞書順最小
    // O(|t|log|S|)
    int lower_bound(std::vector< T >& t)
    {
    ...
    // O(|t|log|S|)
    std::pair<int, int> lower_upper_bound(std::vector< T >& t) {
...
public:
    ...
    SuffixArray(const std::vector< T >& str) : s(str)
    {
        SA.resize(s.size());
    ...
    // s中のtの位置 
    // 順番は適当 ソートしてない
    std::vector<int> find(std::vector< T > &t) {
        std::vector<int> v;
        std::pair<int, int> p = lower_upper_bound(t);
    ...
    void build_lcp() {
        ...
        for (int i = 0, h = 0; i < (int)s.size(); i++) {
            if (rank[i] + 1 < (int)s.size()) {
                for (int j = SA[rank[i] + 1]; std::max(i, j) + h < s.size() && s[i + h] == s[j + h]; ++h);
                LCP[rank[i] + 1] = h;
                if (h > 0) --h;
            }
        }
